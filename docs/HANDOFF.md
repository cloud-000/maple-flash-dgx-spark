...
endpoint. Quality evals live in the sibling `bench` project and hit that
endpoint (`bench eval --base-url … --model maple-2bit`). Do not re-convert unless the packed checkpoint is
...
| 6 Quality evals | **Moved to `../bench`.** OpenAI-compatible harness (`bench eval`) against `maple-run serve`. Dense-head protocol; full numbers still running. |
...
### Phase 6 — quality evals — **moved to `../bench`**

The sibling `bench` project runs DeepGrove's published table against any
OpenAI-compatible `/v1/chat/completions` server (including `maple-run serve`)
on the **dense** 4-bit head (not FlashHead): LiveCodeBench v6, AIME 2026, HMMT
Feb 2026, GPQA-Diamond. Protocol: T=1.0 top_p=0.95 top_k=20, n=4,
max_tokens=64000, MathArena boxed grading, simple-evals GPQA shuffle, official
LCB tests. Resume under `--output`. DeepGrove's numbers: LCB 75.1, AIME 87.5,
HMMT 78.8, GPQA-D 73.5 (mean 78.7).
...
| Path | Role |
|---|---|
| `src/maple_run/cli.py` | `convert` / `generate` / `serve` subcommands (`eval` points at `../bench`) |
...
| `src/maple_run/eval_score.py` | Boxed / AIME extractors used by EGGROLL rewards |
| `tests/test_eval_moved.py` | CLI `eval` points at the `bench` project |
...
4.57.1). Eval extra is `datasets` + `sympy` for EGGROLL dataset envs (IPI).
Quality benches live in `../bench`.
